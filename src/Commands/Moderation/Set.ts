    ...
    public override execute = async (M: Message, { flags }: IArgs): Promise<void> => {
        ...
        if (!flags.length) {
            ...
            if ((action === 'true' && data[feature]) || (action === 'false' && !data[feature]))
                return void M.reply(
                    `🟨 *${this.client.utils.capitalize(feature)} is already ${action === 'true' ? 'Enabled' : 'Disabled'
                    }*`
                )
            await this.client.DB.updateGroup(M.from, feature, action === 'true')
            return void M.reply(
                `${action === 'true' ? '🟩' : '🟥'} *${this.client.utils.capitalize(feature)} is now ${action === 'true' ? 'Enabled' : 'Disabled'
                }*`
            )
        }
    }
