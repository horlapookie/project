...
module.exports = {
  ...
  usage: 'Use {prefix}yucollection [index]',
  description: 'View your Yu-Gi-Oh collection',
  async execute(client, arg, M) {
    const userKey = getUserKey(client, M)
    const collection = await getCollection(client, userKey)
    if (!collection.length) return M.reply('Your Yu-Gi-Oh collection is empty.')

    const idx = parseInt(String(arg || '').trim(), 10)
    if (idx) {
      const card = collection[idx - 1]
    ...
    let text = `🃏 *Yu-Gi-Oh Collection* (Total: ${collection.length})\n\n`
