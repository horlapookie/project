    ...
    public start = async (): Promise<client> => {
        ...
        this.ev.on('messages.upsert', async ({ messages }) => {
            ...
            if (M.stubType && M.stubParameters) {
                ...
                // Handle group events based on stub type numbers
                const stubType = M.stubType as number
                if (stubType === 1) {
                    // GROUP_CREATE
                ...
                } else if ([24, 32, 2].includes(stubType)) {
                    // GROUP_PARTICIPANT_ADD, ADD_REQUEST_JOIN, INVITE
                    return void emitParticipantsUpdate('add')
                } else if ([25, 26].includes(stubType)) {
